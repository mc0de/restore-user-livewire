<form wire:submit.prevent="submit" class="pt-3">
    ...
    <div class="form-group {{ $errors->has('competence') ? 'invalid' : '' }}">
        <label class="form-label" for="competence">{{ trans('cruds.user.fields.competence') }}</label>
        <x-select-list class="form-control" id="competence" name="competence" wire:model="competence" :options="$this->listsForFields['competence']" multiple />
        <div class="validation-message">
            {{ $errors->first('competence') }}
        </div>
        <div class="help-block">
            {{ trans('cruds.user.fields.competence_helper') }}
        </div>
    </div>
